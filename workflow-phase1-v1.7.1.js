(function(root,factory){
  ...
  function todayEvents(state=stateNow(),options={}){
    ...
    const add=e=>{if(!e||!e.key)return;if(events.some(x=>x.key===e.key))return;events.push(e);};
  ...
  function nav(route){
    const button=root.document?.querySelector(`.nav-item[data-route="${route}"]`);if(button){button.click();return true;}return false;
  }
  function waitFor(selector,fn,attempt=0){
    const node=root.document?.querySelector(selector);
    if(node){fn(node);return;}
    if(attempt>=40)return;
    root.setTimeout?.(()=>waitFor(selector,fn,attempt+1),50);
  }
  function setFormValue(formSelector,name,value){
    waitFor(formSelector,form=>{
      const field=form.querySelector(`[name="${name}"]`);if(!field)return;
  ...
  function openCoreHealth(animalId,type='Observation'){
    closeCoreModal();
    if(!nav('health'))return toast('Health is not available right now.','error');
    root.setTimeout?.(()=>{
      const add=root.document.querySelector('#add-health');if(!add)return toast('Health entry did not load. Try again.','error');
      add.click();setFormValue('#health-form','animalId',animalId);setFormValue('#health-form','type',type);
    },0);
  }
  function openHealthIntelligence(animalId,action='episode'){
    closeCoreModal();
    if(!root.HerdHarborHealthIntelligence)return openCoreHealth(animalId,'Observation');
    const trigger=root.document.createElement('button');trigger.type='button';trigger.dataset.hiAction=action;trigger.hidden=true;root.document.body.appendChild(trigger);trigger.click();trigger.remove();
    setFormValue('#hh-health-intelligence-modal form','animalId',animalId);
  }
  function openBreeding(animal){
    if(!animal)return;
    if(!isCurrentAnimal(animal))return toast('Historical animals cannot start a new breeding.','error');
    if(isQuarantined(animal.id))return toast(`${animal.name||'This animal'} is quarantined. Clear the active quarantine before starting a breeding.`,'error');
    if(!['female','male'].includes(lower(animal.sex)))return toast('Record the animal sex before starting a breeding.','error');
    closeCoreModal();if(!nav('breeding'))return toast('Breeding is not available right now.','error');
    root.setTimeout?.(()=>{
      const add=root.document.querySelector('#add-breeding');if(!add)return toast('Breeding entry did not load. Try again.','error');
      add.click();setFormValue('#breeding-form',lower(animal.sex)==='female'?'femaleId':'maleId',animal.id);
    },0);
  }
  function openShowEntry(animalId=''){
    closeCoreModal();
    const showNav=root.document.querySelector('.nav-item[data-route="shows"]');
    if(!showNav)return toast('Shows is still loading. Try again in a moment.','error');
    showNav.click();
    waitFor('[data-add-entry]',button=>{button.click();if(animalId)setFormValue('#hh-entry-form','animalId',animalId);});
  }
  ...
  function recordList(rows,emptyText){
    if(!rows.length)return`<p class="muted">${esc(emptyText)}</p>`;
    return`<div class="hh-p1-record-list">${rows.map(r=>`<article><strong>${esc(r.title)}</strong><span>${esc(r.meta||'')}</span>${r.detail?`<small>${esc(r.detail)}</small>`:''}</article>`).join('')}</div>`;
  }
  function healthPanel(model){
    const rows=[
      ...model.health.legacy.slice().sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).slice(0,6).map(r=>({title:r.type||'Health record',meta:fmtDate(r.date),detail:r.details||r.notes||''})),
      ...model.health.episodes.slice().sort((a,b)=>String(b.startedDate||'').localeCompare(String(a.startedDate||''))).slice(0,4).map(r=>({title:r.concern||'Health episode',meta:`${fmtDate(r.startedDate)} · ${r.assessment?.level||r.healthStatus||''}`,detail:r.resolved?'Resolved':r.quarantined?'Quarantined':'Open'})),
      ...model.health.care.slice().sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).slice(0,4).map(r=>({title:r.product||r.type||'Care record',meta:`${fmtDate(r.date)} · ${r.type||'Care'}`,detail:r.reason||r.notes||''}))
    ].slice(0,10);
    return`<div class="hh-p1-panel-head"><div><h3>Health</h3><p>${model.health.legacy.length+model.health.episodes.length+model.health.care.length+model.health.groups.length} connected health records.</p></div><div><button class="button button-primary button-small" data-hh-p1-action="weight">Add weight</button><button class="button button-ghost button-small" data-hh-p1-action="episode">Start episode</button><button class="button button-ghost button-small" data-hh-p1-action="care">Add care</button></div></div>${recordList(rows,'No health records are connected to this animal yet.')}`;
  }
  function breedingPanel(model,state){
    const rows=model.breedings.slice().sort((a,b)=>String(b.breedingDate||'').localeCompare(String(a.breedingDate||''))).map(r=>({title:`${animalName(state,r.femaleId)} × ${animalName(state,r.maleId)}`,meta:`${fmtDate(r.breedingDate)} · ${r.status||'Bred'}`,detail:r.dueDate?`Due ${fmtDate(r.dueDate)}`:''}));
    const blocked=model.quarantined?'Breeding is unavailable while this animal is quarantined.':!model.current?'Historical animals cannot start a new breeding.':'';
    return`<div class="hh-p1-panel-head"><div><h3>Breeding</h3><p>${blocked||'Existing HerdHarbor breeding records stay authoritative.'}</p></div>${!blocked?`<button class="button button-primary button-small" data-hh-p1-action="breeding">Start breeding</button>`:''}</div>${recordList(rows,'No breeding history is connected to this animal.')}`;
  }
  ...
  function showsPanel(model,state){
    const rows=model.shows.entries.slice().sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).slice(0,8).map(e=>{const show=array(state,'shows').find(s=>String(s.id)===String(e.showId));return{title:show?.name||'Show entry',meta:fmtDate(show?.startDate||e.date),detail:e.className||e.division||''};});
  ...
  function productionPanel(model){
    const rows=model.production.slice().sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).slice(0,10).map(r=>({title:r.product||r.type||'Production',meta:fmtDate(r.date),detail:[r.amount??r.quantity,r.unit].filter(v=>v!==undefined&&v!==null&&v!=='').join(' ')}));
  ...
  function renderPanel(id,model,state){
    if(id==='health')return healthPanel(model);
    if(id==='breeding')return breedingPanel(model,state);
    if(id==='genetics')return geneticsPanel(model);
    if(id==='pedigree')return pedigreePanel(model,state);
    if(id==='shows')return showsPanel(model,state);
    if(id==='production')return productionPanel(model);
    if(id==='history')return historyPanel(state,model.animal.id);
    return'';
  }
  function actionButtons(model){
    const a=model.animal,buttons=[];
    buttons.push(['weight','Add weight','primary']);
    buttons.push(['episode','Health episode','ghost']);
    if(model.current&&['female','male'].includes(lower(a.sex))&&!model.quarantined)buttons.push(['breeding','Breed','ghost']);
    if(model.tabs.includes('genetics'))buttons.push(['genetics','Genetics','ghost']);
    buttons.push(['pedigree','Pedigree','ghost']);
    if(model.tabs.includes('shows'))buttons.push(['show-entry','Show entry','ghost']);
    buttons.push(['edit','Edit / status','ghost']);
    return buttons.map(([action,label,tone])=>`<button type="button" class="button button-${tone} button-small" data-hh-p1-action="${action}">${esc(label)}</button>`).join('');
  }
  function resolveProfileAnimalId(content){
    const state=stateNow();
    if(pendingAnimalId&&animalById(state,pendingAnimalId))return pendingAnimalId;
    const query=new URLSearchParams(root.location?.search||'').get('animal');if(query&&animalById(state,query))return query;
  ...
  function enhanceAnimalProfile(){
    ...
    pendingAnimalId=id;content.dataset.hhPhase1AnimalId=id;
  ...
  function dashboardEventHtml(event,state){
    const animal=event.animalId?animalName(state,event.animalId):'';
    const badge=event.tone==='overdue'?'Overdue':event.tone==='today'?'Today':event.tone==='attention'?'Attention':'Soon';
    return`<article class="hh-p1-today-item is-${event.tone}"><div class="hh-p1-today-date">${event.date?esc(fmtDate(event.date)):'Now'}</div><div class="hh-p1-today-copy"><strong>${esc(event.title)}</strong><span>${esc([event.detail,animal].filter(Boolean).join(' · '))}</span></div><span class="badge ${event.tone==='overdue'||event.tone==='attention'?'danger':event.tone==='today'?'green':'warning'}">${badge}</span><button type="button" class="button button-ghost button-small" data-hh-p1-event="${esc(event.key)}">Open</button></article>`;
  }
  function enhanceDashboard(){
    const view=root.document?.querySelector('#view-dashboard');if(!view||!view.classList.contains('active')||view.querySelector('#hh-p1-today'))return false;
    const state=stateNow(),events=todayEvents(state),overdue=events.filter(e=>e.tone==='overdue').length,due=events.filter(e=>e.tone==='today').length,attention=events.filter(e=>e.tone==='attention').length,soon=events.filter(e=>e.tone==='soon').length;
    const panel=root.document.createElement('section');panel.id='hh-p1-today';panel.className='panel hh-p1-today';
    ...
    const header=view.querySelector('.page-header');if(header)header.insertAdjacentElement('afterend',panel);else view.prepend(panel);
    view.querySelector('.task-today-panel')?.classList.add('hh-p1-task-panel-hidden');
    panel.addEventListener('click',e=>{
      if(e.target.closest('[data-hh-p1-open-tasks]')){root.document.querySelector('#dashboard-view-tasks')?.click();return;}
      if(e.target.closest('[data-hh-p1-quick-add]')){root.document.querySelector('#quick-add-button')?.click();return;}
      const key=e.target.closest('[data-hh-p1-event]')?.dataset?.hhP1Event;if(!key)return;const event=events.find(x=>x.key===key);if(!event)return;
      if(event.source==='health')nav('health');else if(event.source==='shows')root.document.querySelector('.nav-item[data-route="shows"]')?.click();else nav('tasks');
    });
    return true;
  }
  function enhanceQuickAdd(){
    const content=root.document?.querySelector('#modal-content'),title=clean(root.document?.querySelector('#modal-title')?.textContent);if(!content||title!=='Quick add'||content.querySelector('.hh-p1-quick-context'))return false;
    const route=root.HerdHarborApp?.getCurrentRoute?.()||'dashboard',types=contextualQuickTypes(route),labels={animal:'Animal',health:'Health',task:'Task',pedigree:'Pedigree',breeding:'Breeding',litter:'Birth / litter',expense:'Expense',income:'Income',sale:'Animal sale',customer:'Customer'};
    const block=root.document.createElement('section');block.className='hh-p1-quick-context';block.innerHTML=`<p class="eyebrow">Suggested here</p><div>${types.map(type=>`<button type="button" class="button button-ghost button-small" data-hh-p1-quick="${type}">${esc(labels[type]||type)}</button>`).join('')}</div>`;content.prepend(block);
    block.addEventListener('click',e=>{const type=e.target.closest('[data-hh-p1-quick]')?.dataset?.hhP1Quick;if(!type)return;content.querySelector(`[data-quick="${type}"]`)?.click();});return true;
  }
  function enhance(){enhanceAnimalProfile();enhanceDashboard();enhanceQuickAdd();}
  function scheduleEnhance(){if(queued)return;queued=true;const run=()=>{queued=false;enhance();};if(typeof root.queueMicrotask==='function')root.queueMicrotask(run);else root.setTimeout?.(run,0);}
  function install(){
    if(root.__hhPhase1WorkflowInstalled)return API;root.__hhPhase1WorkflowInstalled=true;
    root.document.addEventListener('click',e=>{const view=e.target.closest?.('[data-view-animal]');if(view?.dataset?.viewAnimal)pendingAnimalId=String(view.dataset.viewAnimal);},true);
    ...
    if(typeof root.MutationObserver==='function'&&root.document.body){observer=new root.MutationObserver(scheduleEnhance);observer.observe(root.document.body,{childList:true,subtree:true});}
    scheduleEnhance();return API;
  }
  function uninstall(){observer?.disconnect?.();observer=null;root.__hhPhase1WorkflowInstalled=false;}
  const API=Object.freeze({VERSION,CONTRACT,isCurrentAnimal,profileTabs,profileModel,timelineRows,contextualQuickTypes,todayEvents,enhanceAnimalProfile,enhanceDashboard,enhanceQuickAdd,install,uninstall});
  return API;
});
