(function (root) {
  ...
  root.HerdHarborBuild = Object.freeze({
    ...
    channel: "Alpha",
    version: "1.8.2",
    buildId: "cloud-sync-v2-baseline-recovery-1",
    build: "1.8.2-alpha-cloud-sync-v2-baseline-recovery-1"
  });
    ...
    StorageCtor.prototype.setItem = function herdHarborCloudSyncV2SetItem(key, value) {
      if (this === storage && key === CLOUD_STATE_KEY) {
        try {
          ...
          const baseMissing = userId && !storage.getItem(cloudBaseKey(userId));
          const clean = userId && storage.getItem(cloudDirtyKey(userId)) !== "1";
          const versionKnown = userId && Boolean(storage.getItem(cloudVersionKey(userId)));
          if (
            userId &&
            baseMissing &&
            clean &&
            versionKnown &&
            previousRaw !== value &&
            isReadableCloudState(previousRaw)
          ) {
            originalStorageSetItem.call(storage, cloudBaseKey(userId), previousRaw);
            try {
              root.dispatchEvent?.(new CustomEvent("herdharbor:cloud-baseline-restored", {
                detail: { userId, reason: "before-local-edit" }
              }));
            } catch {}
          }
        } catch {}
