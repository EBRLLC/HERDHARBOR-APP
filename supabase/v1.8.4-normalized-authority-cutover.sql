...
--
-- No account is promoted by this migration. It adds guarded RPCs and hardens
-- existing stage/write paths. The internal validation cohort remains empty
-- unless an operator explicitly enrolls a user outside the browser.
...
-- PR7 manifest guard: forward rollout requires the server cohort, final cutover
-- requires current checkpoint verification + prepared writer + authority marker,
-- while ordinary record-CAS writes in normalized stage may invalidate the
-- global checkpoint without invalidating normalized authority.
...
-- Re-harden record CAS for normalized authority. Existing shadow/dual-write
-- semantics are retained, but normalized writes additionally require the
-- authority marker and cannot update a tombstoned row back to active.
...
comment on function public.herdharbor_sync_materialize_legacy_recovery(
  ...
  'Writes one generation-guarded normalized snapshot into the legacy compatibility row before rollback, without reopening ordinary stale legacy writes.';
