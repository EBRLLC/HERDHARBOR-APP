...
const HARD_LAUNCH_AT = "2026-10-01T04:00:00.000Z";
...
const launchPolicy = () => ({
  version: "1.8.1",
  phase: Date.now() < HARD_LAUNCH_MS ? "launch_trial" : "subscriptions_live",
...
async function buildSnapshot(admin: ReturnType<typeof createClient>, userId: string) {
  ...
  const rawStatus = String(sub?.status || "not_configured");
  const trialEndsAt = sub?.trial_ends_at || null;
  const trialExpired = rawStatus === "trialing" && Boolean(trialEndsAt) && new Date(String(trialEndsAt)).getTime() <= Date.now();
  return {
    status: trialExpired ? "expired" : rawStatus,
    plan: trialExpired ? null : (sub?.plan_id || null),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "policy") return json(launchPolicy());

    const { data: current, error: currentError } = await admin.from("subscriptions").select("*").eq("user_id", user.id).maybeSingle();
    if (currentError) throw currentError;

    if (action === "checkout") {
      if (Date.now() < HARD_LAUNCH_MS) {
        return json({
          error: "Subscriptions launch October 1. Your HerdHarbor Launch Trial stays free through September 30.",
          launchPolicy: launchPolicy()
        }, 409);
      }
