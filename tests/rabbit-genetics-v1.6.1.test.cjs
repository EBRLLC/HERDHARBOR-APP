...
test('pair analysis calculates modifier probabilities and flags health-linked combinations',()=>{
  const core={A:['a','a'],B:['B','B'],C:['C','C'],D:['D','D'],E:['E','E']};
  const animal=(name,loci)=>({id:name,name,species:'Rabbit',genetics:{loci:{...core,...loci}}});
  const result=Engine.analyzePairing(animal('Buck',{En:['En','en'],Dw:['Dw','dw']}),animal('Doe',{En:['En','en'],Dw:['Dw','dw']}));
...
test('canonical phenotype is distinct from recorded and registry terminology',()=>{
  const result=Engine.canonicalPhenotype({phenotype:{recorded:'local black'},loci:{A:['a','a'],B:['B','B'],C:['C','C'],D:['D','D'],E:['E','E'],En:['En','en']}});
  ...
  assert.match(Engine.REGISTRIES.arba.scope,/not a judging/);
});
