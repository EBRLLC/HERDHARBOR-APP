...
const pagesWorkflow = read(".github/workflows/v1.8.2-production-pages.yml");
const reviewWorkflowPath = ".github/workflows/v1.8.2-ci.yml";
const productionWorkflowPath = ".github/workflows/v1.8.2-production-acceptance.yml";
...
assert.match(config, /HerdHarbor@1\.8\.2/);
assert.match(config, /cloud-sync-v2-state-integrity-1/);
...
assert.match(pagesWorkflow, /npm run test:v1\.8\.2/);
...
assert.ok(fs.existsSync(path.join(root, reviewWorkflowPath)), "v1.8.2 CI workflow must exist");
const reviewWorkflow = read(reviewWorkflowPath);
assert.match(reviewWorkflow, /name: Alpha v1\.8\.2 CI/);
assert.match(reviewWorkflow, /android-review:/);
assert.match(reviewWorkflow, /\.\/gradlew --no-daemon bundleRelease/);
assert.match(reviewWorkflow, /herdharbor-v1\.8\.2-unsigned-aab/);
...
assert.ok(fs.existsSync(path.join(root, productionWorkflowPath)), "protected v1.8.2 production acceptance workflow must exist");
...
console.log("Alpha v1.8.2 deployment, Android review, protected production acceptance, and IndexedDB guardrails passed");
