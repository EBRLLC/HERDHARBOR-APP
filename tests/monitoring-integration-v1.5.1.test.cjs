...
assert.doesNotMatch(core, /replayIntegration\s*\(/i);
assert.doesNotMatch(core, /browserTracingIntegration\s*\(/i);
assert.doesNotMatch(core, /enableLogs:\s*true/);
assert.doesNotMatch(core, /tracesSampleRate:\s*(?:[1-9]|0\.[1-9])/);
assert.doesNotMatch(core, /metrics\./i, "Application Metrics remain off");
...
assert.doesNotMatch(instrumentation, /value\s*\)/, "storage instrumentation does not inspect stored values");
...
assert.match(worker, /v1\.5\.1-alpha-monitoring-review-1/);
