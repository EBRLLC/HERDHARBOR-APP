...
assert.match(config, /HerdHarbor@1\.6\.1/);
...
assert.match(core, /"admin"/);
assert.match(core, /"membership"/);
assert.match(core, /"subscription"/);
...
assert.ok(shows.includes("const VERSION='1.6.1'"), "Shows runs from the v1.6.1 runtime");
