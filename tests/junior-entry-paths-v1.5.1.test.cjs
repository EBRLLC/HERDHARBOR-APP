...
const gates = (appRuntime + "\n" + animalProfileRuntime + "\n" + breedingLitterRuntime + "\n" + salesCustomerRuntime).match(/allowsAnimalTransition\(/g) || [];
...
assert.match(salesCustomerRuntime, /handleTransferImport[\s\S]*?allowsAnimalTransition\(stateNow\(\)\.animals, \[\.\.\.stateNow\(\)\.animals, \.\.\.added\]\)/, "transfer import is gated");
...
assert.match(appRuntime, /const APP_VERSION = window\.HerdHarborBuild\?\.version \|\| "1\.8\.4";/, "embedded app metadata is Alpha v1.8.4");
