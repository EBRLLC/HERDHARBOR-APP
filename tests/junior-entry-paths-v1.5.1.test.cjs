...
assert.match(html, /const APP_VERSION = "1\.6\.1";/, "embedded app metadata is Alpha v1.6.1");
assert.match(cloud, /version: "1\.6\.1"[\s\S]*?backupType: "local-safety-backup"/, "safety backups identify Alpha v1.6.1");
...
console.log("Alpha v1.6.1 Junior animal-entry and data-preservation tests passed");
