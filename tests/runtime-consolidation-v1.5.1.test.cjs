...
for (const file of activeV161) {
  ...
  assert.ok((pwa + "\n" + html).includes(file + "?v=1.7.0") || html.includes(file + "?v=1.6.5"), "startup loader does not load current runtime asset " + file);
  assert.ok(worker.includes("./" + file + "?v=1.7.0"), "service-worker.js does not precache current runtime asset " + file);
}
...
console.log("v1.7.0 runtime consolidation tests passed");
