...
assert.match(appRuntime, /let animalView = \{[\s\S]*?status: "Active"[\s\S]*?\};/);
assert.match(appRuntime, /id="animal-status"[\s\S]*?"Ancestor Only"/);
assert.match(appRuntime, /animalView\[fieldName\] = event\.currentTarget\.value/);
assert.match(appRuntime, /id="task-status-filter"/);
assert.match(appRuntime, /id="task-custom-interval"/);
assert.match(appRuntime, /function ensureNextRecurringTask\(/);
assert.match(appRuntime, /id="add-production"/);
assert.match(appRuntime, /id="repeat-last-production"/);
assert.match(appRuntime, /id="download-production-report"/);
assert.match(appRuntime, /id="production-report-group"/);
