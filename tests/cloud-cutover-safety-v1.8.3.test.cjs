...
const guardSchema = fs.readFileSync(path.join(root, "supabase", "v1.8.3-cloud-sync-cutover-legacy-guard.sql"), "utf8");
...
test("base SQL owns writer readiness and the cutover guard owns its browser wrapper",()=>{assert.match(schema,/herdharbor_sync_prepare_normalized_writer\(/i);assert.match(guardSchema,/herdharbor_sync_prepare_normalized_writer_guarded/i);assert.match(guardSchema,/return public\.herdharbor_sync_prepare_normalized_writer/i);assert.match(schema,/v_stage <> 'dual_write'/i);assert.match(schema,/HH_SYNC_DUAL_WRITE_STAGE_REQUIRED/);assert.match(schema,/HH_SYNC_WRITER_FORMAT_MISMATCH/);assert.match(schema,/normalized_verified_at = null/i);assert.match(schema,/'normalized_writer_ready', true/i);assert.match(schema,/'normalized_writer_version', btrim\(p_writer_version\)/i);assert.match(schema,/sync_generation = sync_generation \+ 1/i);});
