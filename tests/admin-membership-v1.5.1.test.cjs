...
const detailMarkup = admin.slice(admin.indexOf("function detailMarkup"), admin.indexOf("async function openMember"));
assert.match(detailMarkup, /const roleControls = owner\s*\n\s*\?[^:]+Owner role is protected by Supabase/s);
assert.match(detailMarkup, /\$\{roleControls\}[\s\S]*form id="hh-admin-membership-form"/);
...
assert.match(appRuntime, /const defaultState = \{/);
assert.doesNotMatch(appRuntime, /juniorAnimals|juniorPedigrees|juniorSync/, "Junior reuses current farm records and sync");
