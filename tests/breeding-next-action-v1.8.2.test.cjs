...
test('release loader includes the next-action engine under the promoted v1.8.2 identity',()=>{
  ...
  assert.match(build,/version:\s*"1\.8\.2"/);assert.doesNotMatch(build,/version:\s*"1\.8\.1"/);
});
