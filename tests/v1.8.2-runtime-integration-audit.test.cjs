...
function releaseVersion(source) {
  const constRelease = source.match(/const\s+RELEASE\s*=\s*["']([^"']+)["']/)?.[1];
  const objectRelease = source.match(/release\s*:\s*["']([^"']+)["']/)?.[1];
...
test("formal v1.8.4 app release preserves v1.8.2 Cloud Sync V2 component identities", () => {
  ...
  assert.match(build, /build:\s*"1\.8\.3-alpha-v1\.8\.4-release-1"/);
...
test("legacy 1.8.1 references are allowed only as intentionally carried-forward packaged/runtime dependencies", () => {
  ...
  assert.equal(releaseVersion(localCache), "1.8.2");
});
