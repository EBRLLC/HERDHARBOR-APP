...
const releaseNotes = fs.readFileSync(path.join(root, "RELEASE_NOTES-v1.4.0.md"), "utf8");

// v1.4.0 is additive: the proven v1.3.0 Member cattle workflow remains intact.
...
assert.match(releaseNotes, /Business[\s\S]*Alpha v1\.5\.0/);
assert.match(worker, /breeding-intelligence-core\.js/);

console.log("v1.3.0 Member regression remains intact under Alpha v1.4.0");
