...
const currentUi = fs.readFileSync(path.join(root, "rabbit-genetics-ui-compat-v1.6.1.js"), "utf8");
const v2Ui = fs.readFileSync(path.join(root, "rabbit-genetics-ui-advanced-v1.6.1.js"), "utf8");
const intelligence = fs.readFileSync(path.join(root, "breeding-intelligence-v1.6.1.js"), "utf8");
const tools = fs.readFileSync(path.join(root, "breeding-intelligence-tools-v1.6.1.js"), "utf8");
const styles = fs.readFileSync(path.join(root, "breeding-intelligence-v1.6.1.css"), "utf8");
...
assert.equal(first.appVersion, "1.6.1");
...
test("the live Breeding Intelligence save path appends immutable snapshots to protected farm state", async () => {
  ...
  const liveSave = new Function(
    ...
    Core,
    { HerdHarborPWA: { version: "1.6.1", build: "qa-build" } },
  ...
  assert.equal(farmState.breedingIntelligence.predictions[0].metadata.appVersion, "1.6.1");
