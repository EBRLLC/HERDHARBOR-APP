# HerdHarbor Alpha v1.8.4 Acceptance Checklist

This checklist covers the Alpha v1.8.4 release candidate and its production-stability acceptance gates. Automated checks are authoritative where available; manual checks cover browser/device behavior that CI cannot fully prove.
...
- Confirm only the three current v1.8.4 GitHub Actions workflows remain active in the repository tree.
...
- Reload `https://app.herdharbor.com` after deployment and confirm the application identifies v1.8.4 and the current PWA shell.
