...
export function createHerdHarborMonitoring(sentrySdk, runtime = globalThis) {
  ...
  function installBrowserInstrumentation() {
    ...
    runtime.document?.addEventListener?.("herdharbor:account-operation-failure", (event) => {
      const operation = sanitizeDiagnosticMetadata(event?.detail || {}).operation || "account_operation";
      const module = String(operation).startsWith("admin_") ? "admin" : "membership";
      captureOperationalFailure("membership_operation_failure", { module, operation, result: "failure", online: runtime?.navigator?.onLine !== false });
    });
